...
def think(state, quip):
    ...
    while True:
        ...
        while new_state.get_moves() != []: # while state is non-terminal
            state.do_move(random.choice(new_state.get_moves()))

        # Backpropagate
        while node != None: # backpropagate from the expanded node and work back to the root node
            nameDict = { "red": "blue", "blue" : "red"}
            
            name = nameDict[node.who]

            

            temp_score = 0
            if( new_state.get_score()[ name] > new_state.get_score()[node.who]):
                temp_score = 1
            elif( new_state.get_score()[ name] < new_state.get_score()[ node.who]):
                temp_score = -1
